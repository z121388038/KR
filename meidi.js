...
cron: 50 * * * *
 */
const $ = new Env('美的积分秒杀');
const notify = $.isNode() ? require('./sendNotify') : '';
// https://h5vip.midea.cn/promote/w/build/index/awTfmdaS0xWQ9NDMxMQ?mtag=23660001.1.1
// https://mvip.midea.cn/my/score_welfare/exchange?exchange_id=c210dc5462767fb0a856

let UA = encodeURIComponent(`Mozilla/5.0 (Linux; Android 8.0.0; SM-G955U Build/R16NW) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Mobile Safari/537.36`)
let ExchangeId = '';

let CK = 'usrc=1.1.1.2.2.21.1.1; env=%7B%7D; _tea_utm_cache_10000027=undefined; midea_mk=1de7c6dcdcc8f05de13448d11f0d634; Hm_lvt_1b6c31040318bd4e50884394b930004c=1669906135; __tea_cookie_tokens_10000027=%257B%2522web_id%2522%253A%25227172192232919797251%2522%252C%2522ssid%2522%253A%252227338d23-1daf-40c4-9d1f-6a354914403c%2522%252C%2522user_unique_id%2522%253A%25224b7f0c7de6484b92a66c8bd807cf45b8%2522%252C%2522timestamp%2522%253A1669906142605%257D; score_welfare_user=1; Hm_lpvt_1b6c31040318bd4e50884394b930004c=1669906229; uid=209792080; sukey=CzRK983M8y; uin=209839487; mtag=40004.1.10133%3B30045.1.10'
...
startTime = date.getTime() - 10

!(async () => {

    await sign();
    await queryproductlistbyuser();
    if (ExchangeId) {
        console.log("抢购id为"+ExchangeId)
    }else{
        console.log("没有抢购的东西自动退出")
        return
    }
    ...
    if (m === 0) {
        ...
        let waitSec = startTime - new Date().getTime() - 1000
        console.log(`等${waitSec}豪秒到整点`)
        await $.wait(waitSec)
    }
    await run1yuan();
...
async function sign() {
    try {
        let url = `https://mvip.midea.cn/act/taobao/api/sign?channel=mj`
        await taskGet(url)
    } catch (e) {
        $.logErr(e);
    }
...
async function getorderlist() {
    try {
        let goodsList = await taskGet(`https://mvip.midea.cn/next/score_product_r/getorderlist?pagenum=1&pagesize=10`,true)
        let vecOrderList = goodsList.data.vecOrderList
        for (let i = 0; i < vecOrderList.length; i++) {
            vecOrder = vecOrderList[i]
            if (date.getDate() == new Date(vecOrder.CreatedAt).getDate()) {
                console.log('抢到！！！！',vecOrder.Name)
                await notify.sendNotify(`京东白嫖榜`, `【账号】1.gaoyucindy    美的抢到${vecOrder.Name}`)
            }
        }
    } catch (e) {
        $.logErr(e);
...
async function queryproductlistbyuser() {
    try {
        let goodsList = await taskGet(`https://mvip.midea.cn/next/score_product_r/queryproductlistbyuser?nChannel=6&nScoreOrder=0&lColumnId=88&nCard=0&nPageNum=1&nPageSize=10`,false)
        let vecProductList = goodsList.data.userProductList.vecProductList
        for (let i = 0; i < vecProductList.length; i++) {
            vecProduct = vecProductList[i]
            if (date.getTime() == vecProduct.lStartTime * 1000 && vecProduct.strName.indexOf("券")==-1) {
                ExchangeId = vecProduct.strExchangeId
                break
            }
        }
    } catch (e) {
...
async function run1yuan() {
    $.has = false

    await getRandstring()

    if (startTime > new Date().getTime()) {
        let waitSec = startTime - new Date().getTime()
        await $.wait(waitSec)
    }
...
async function getopinfo() {
    try {
        let url = `https://mvip.midea.cn/next/score_product_r/getopinfo?channel=5&exchangeid=${ExchangeId}`
        let data = await taskGet(url)
        $.has = data.data.nStatus != 0 &&  data.data.nStatus != 2
    } catch (e) {
        $.logErr(e);
    }
}
...
async function qiang() {
    try {
        let Rand = $.randstring.data.Rand
        let url = `https://mvip.midea.cn/next/memflashsale/doflashsale?Channel=5&MailName=%E9%BB%84%E5%A5%B3%E5%A3%AB&MailMobile=18695611291&MailAddr=%E4%BA%A4%E9%80%9A%E8%B7%AF%E4%B8%89%E6%BA%90%E8%8A%B1%E5%9B%AD2#608&MailArea=%E7%A6%8F%E5%BB%BA+%E7%A6%8F%E5%B7%9E%E5%B8%82+%E5%8F%B0%E6%B1%9F%E5%8C%BA+%E4%B8%8A%E6%B5%B7%E8%A1%97%E9%81%93+&ProvinceCode=16&CityCode=1303&CountryCode=3483&TownCode=59646&Rand=${Rand}&ExchangeId=${ExchangeId}`
...
async function getRandstring() {
    try {
        let url = `https://mvip.midea.cn/next/memflashsale/getrandstring`
        $.randstring = await taskGet(url)
    } catch (e) {
        $.logErr(e);
    }
}

function taskGet(type, print=true) {
    return new Promise(async (resolve) => {
        let myRequest = getGetRequest(type)
        if(print)console.log('start-' + new Date().getSeconds() + '.' + new Date().getMilliseconds())
        $.get(myRequest, async (err, resp, _data) => {
            let data = ''
            if(print)console.log('end-' + new Date().getSeconds() + '.' + new Date().getMilliseconds())
            if(print)console.log(_data)
            try {
                let contents = ''
                // console.log(_data)
                data = $.toObj(_data)
